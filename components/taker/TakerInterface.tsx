...
import { Upload, ArrowLeft, Lock, Loader2, ChevronRight, Clock, AlertCircle, Check, Bookmark, RotateCcw, Menu, X, Trophy, AlertTriangle, FileQuestion, CheckCircle2, Pause, Play } from 'lucide-react';
  ...
  return (
    <>
      <div className="flex flex-col items-center justify-center h-full w-full bg-zinc-950 text-zinc-300 relative font-sans overflow-hidden">
        ...
        <button onClick={onExit} className="fixed top-4 left-4 flex items-center gap-2 px-3 py-1.5 rounded-lg bg-zinc-900/50 hover:bg-zinc-900 text-zinc-500 hover:text-zinc-200 border border-zinc-800 transition-all text-[10px] font-bold shadow-sm z-10 uppercase tracking-widest"><ArrowLeft size={12} />EXIT</button>
        <div className="w-full max-w-md p-8 rounded-2xl border border-zinc-800 bg-zinc-900/50 backdrop-blur-sm text-center relative z-10"><div className="w-16 h-16 mx-auto bg-zinc-800 rounded-2xl flex items-center justify-center mb-6 shadow-xl border border-zinc-700/50"><Upload size={24} className="text-zinc-400" /></div><h2 className="text-2xl font-bold text-zinc-100 mb-2 uppercase tracking-widest">Load Test Package</h2><p className="text-zinc-500 text-sm mb-8 font-mono uppercase tracking-tighter">Upload the encrypted ZIP provided by your instructor.</p><button onClick={() => fileInputRef.current?.click()} className="w-full py-3 bg-zinc-100 hover:bg-white text-black font-bold rounded-lg transition-all shadow-lg active:translate-y-0.5 uppercase tracking-widest">Select Package</button></div>
      </div>
      {isPasswordModalOpen && (
