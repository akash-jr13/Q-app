...
import { Upload, ArrowLeft, Lock, Loader2, ChevronRight, ChevronLeft, Clock, AlertCircle, Check, Bookmark, RotateCcw, Menu, X, Trophy, AlertTriangle, Send, FileQuestion, CheckCircle2 } from 'lucide-react';
...
export const TakerInterface: React.FC<TakerInterfaceProps> = ({ onExit }) => {
  ...
  if (testData && currentQuestion) {
    ...
    const markedAnsweredCount = testData.questions.filter(q => marked.has(q.id) && answers[q.id]).length;
    ...
    // Sidebar stats object
    const sidebarStats = {
        answered: attemptedCount - markedAnsweredCount, // "Answered" in sidebar usually implies not marked
        notAnswered: visited.size - attemptedCount - (marked.size - markedAnsweredCount), // Logic can be tricky, let's stick to the visual definition
        marked: markedCount - markedAnsweredCount,
        ...
        <div className="h-10 bg-zinc-950 border-b border-zinc-800 flex items-end px-4 gap-2 shrink-0 overflow-x-auto">
             {subjects.map(sub => (
                 <button
                    ...
                    className={`px-6 py-2 rounded-t-lg text-xs font-bold uppercase tracking-wide transition-all border-t border-x border-b-0 translate-y-[1px] ${
                        activeSubject === sub 
                                ...
                                {Array.from({ length: numOptions }).map((_, i) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`h-14 rounded-xl border flex items-center px-4 gap-4 transition-all group relative overflow-hidden ${
                                                isSelected 
                                        ...
                                        >
                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border transition-colors ${
                                                isSelected ? 'border-black bg-black text-white' : 'border-zinc-700 bg-zinc-900 group-hover:border-zinc-500'
                                            }`}>
                                                {label}
                                            </div>
             ...
             <div className="h-16 border-t border-zinc-800 bg-zinc-900 px-6 flex items-center justify-between shrink-0 shadow-2xl z-10">
                 <div className="flex gap-3">
                     ...
                     <button 
                        ...
                        className={`px-4 py-2 rounded-lg border text-xs font-bold flex items-center gap-2 transition-colors ${
                                  ...
                                  {groupedQuestions[groupName].map((q) => {
                                      ...
                                      return (
                                          <button
                                            ...
                                            className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold transition-all relative shadow-sm ${statusClass} ${
                                                isCurrent ? 'ring-2 ring-white scale-110 z-10' : 'hover:scale-105'
                                            }`}
                                          >
                                            {q.questionNumber}
                                            {isMarkedAndAnswered && (
